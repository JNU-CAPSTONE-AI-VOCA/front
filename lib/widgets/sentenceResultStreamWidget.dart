...
class _SentenceResultStreamWidgetState extends State<SentenceResultStreamWidget> {
  ...
  void initState() {
    ...
    _loadJsonData().then((result) {
      setState(() {
        ...
        _loadJsonData();
        completeJsonList = completeVocaList.map((e) {
          return e.toJson();
        }).toList();
  ...
  void sseConnect() async {
    ...
    var response = await Dio().post(
      'http://13.124.234.107:8080/api/generate/sentence/stream',
      data: jsonString,
      options: options,
    );
